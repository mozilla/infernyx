...
zcat all_distinct.gz | grep "$locale,$country" | ./infernyx/tools/histogram.py -k 5 -v 6 | tr "," " " | sort  -k1 -n | tr " " "," > "$locale.$country".hist
# extract tuples for locale+country
zcat all_tuples.gz | grep "$locale,$country" | sed -e 's/,www\./,/g' | ./infernyx/tools/histogram.py -k 5,6 -v 7 -r | sort -t"," -k3 -nr > "$locale.$country".tuple_count
